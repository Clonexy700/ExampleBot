...
DEFAULT_BOT_COLOR = settings['defaultBotColor']
...
def create_blackjack_embed(client, state_of_game: str, player_hand: Hand, dealer_hand: Hand,
    ...
    embed = nextcord.Embed(title='Блекджек', description=state_of_game, color=DEFAULT_BOT_COLOR)
    player_hand_field_value = get_hand_cards(client, player_hand)
    dealer_hand_field_value = get_hand_cards(client, dealer_hand)
    embed.add_field(name='Ваша рука', value=f'{player_hand_field_value}\n'
                                              f'ценность **{player_hand.get_value()}**', inline=True)
    embed.add_field(name='Рука дилера', value=f'{dealer_hand_field_value}\n'
                                              f'ценность **{dealer_hand.get_value()}**', inline=True)
...
def create_game_start_blackjack_embed(client, state_of_game: str, player_hand: Hand, dealer_hand: Hand,
    ...
    embed.add_field(name='Рука дилера', value=f'{dealer_hand_field_value}\nценность **{second_dealer_card}**', inline=True)
    if footer_text is not None and footer_url is not None:
        embed.set_footer(text=footer_text, icon_url=footer_url)
    return embed
...
def create_final_view() -> View:
    hit = create_button("Ещё", False, True)
    stand = create_button("Хватит", False, True)
    view = View()
    view.add_item(hit)
    view.add_item(stand)
    return view
